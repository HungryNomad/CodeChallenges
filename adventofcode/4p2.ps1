...
$lines = $lines -as [System.Collections.ArrayList]

$winner = $null
$ball = 0
while ( $lines -and ($ball -lt $numbers.Count)) {
    ...
    if ($winner) {
        ...
        Write-Output "Card: $($winner.card)  Remaining: $remaining LastNumber: $lastNumber Ball: $($ball-1) Score: $($remaining * $lastNumber)"

        foreach ($winnerID in ($lines | Where-Object { $winner.card -contains $_.card } )) {
            $lines.remove($winnerID)
        }
    }
}
